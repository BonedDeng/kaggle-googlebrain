...
class VentilatorDataset(Dataset):
    def __init__(self, df, train_df, mode = 'train'):
        ...
        self.train_uin, self.train_pressure = process_train_df(train_df)
    ...
    def __getitem__(self, idx):
        ...
        input = np.concatenate([input,train_uin.reshape(-1,1),train_uin_diff1.reshape(-1,1), train_uin_diff2.reshape(-1,1),\
                                train_uin_diff_diff.reshape(-1,1), train_uin_diff1_diff1.reshape(-1,1), train_uin_diff2_diff2.reshape(-1,1), \
                                train_pressure.reshape(-1,1),train_pressure_diff1.reshape(-1,1),train_pressure_diff2.reshape(-1,1)], axis = -1)
...
sub.to_csv(RESULT_PATH+f'{Config.selected_model}_submission.csv', index=False)
...
sub.to_csv(RESULT_PATH+f'{Config.selected_model}_submission_post.csv', index=False)
